...
class DBNet(tf.keras.Model):
    ...
    def _build_model(self, k=50):
        ...
        backbone = ResNet18(image_input, include_top=False)
        ...
        P5 = tf.keras.Sequential(
            [ConvBnRelu(64, 3), UpSampling2D(8)], name='P5')(in5)
        P4 = tf.keras.Sequential(
            [ConvBnRelu(64, 3), UpSampling2D(4)], name='P4')(out4)
        P3 = tf.keras.Sequential(
            [ConvBnRelu(64, 3), UpSampling2D(2)], name='P3')(out3)
        P2 = ConvBnRelu(64, 3, name='P2')(out2)

        # Calculate DBNet maps
        fuse = Concatenate(name='fuse')(
            [P2, P3, P4, P5])  # (batch_size, /4, /4, 256)
    ...
    def predict_one_page(_self, raw_image):
        ...
        batch_boxes, batch_scores = _self.post_processor(
            binarize_map.numpy(), [raw_image.shape[:2]])
