...
class CRNN(tf.keras.Model):
    def __init__(self):
        ...
        self.num2char = tf.keras.layers.StringLookup(
            vocabulary=open('./assets/vocab.txt',
    ...
    def _build_model(self):
        ...
        feature_maps = Reshape(target_shape=(
            (height, width * channel)), name='rnn_input')(x)

        # RNN layers
        bigru1 = Bidirectional(
            GRU(256, return_sequences=True), name='bigru1')(feature_maps)
        bigru2 = Bidirectional(
            GRU(256, return_sequences=True), name='bigru2')(bigru1)

        # Output layer
        y_pred = Dense(self.num2char.vocabulary_size() + 1,
                       activation='softmax', name='rnn_output')(bigru2)
    ...
    def distortion_free_resize(self, image, align_top=True):
        image = tf.image.resize(image, size=(
            self.height, self.width), preserve_aspect_ratio=True)
        ...
        return tf.pad(image, paddings=[
            [0, pad_height_top + pad_height_bottom] if align_top else [pad_height_top,
                                                                       pad_height_bottom],
    ...
    def tokens2texts(self, batch_tokens):
        ...
        for tokens in batch_tokens:
            indices = tf.gather(tokens, tf.where(
                tf.logical_and(tokens != 0, tokens != -1)))
