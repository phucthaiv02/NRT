...
def generate_initial_drawing(boxes, size_ratio):
    ...
    for idx, box_pts in enumerate(boxes):
        left = max(box_pts[0][0], box_pts[3][0])
        top = max(box_pts[0][1], box_pts[1][1])
        width = max(box_pts[1][0], box_pts[2][0]) - \
            min(box_pts[0][0], box_pts[3][0])
        height = max(box_pts[2][1], box_pts[3][1]) - \
            min(box_pts[0][1], box_pts[1][1])
...
def order_boxes4nom(bounding_boxes):
    ...
    # Sort by x2 (right boundary)
    bounding_boxes.sort(key=lambda box: box[1][0], reverse=True)
    ...
    for box in bounding_boxes:
        ...
        for col in columns:
            ...
            # Dynamic threshold based on width
            if abs(box[1][0] - col[-1][1][0]) < abs(box[0][0] - box[1][0]) * 0.5:
                col.append(box)
                placed = True
                break
        if not placed:
